...
let stuScore = studentRecords.filter(item => item.marks > 50 && item.id > 120);
...
let total = studentRecords.reduce((acc,curr) => acc += curr.marks,0)
console.log(total);
...
let marks = studentRecords.filter((item) => item.id>120).reduce((acc,curr) => acc += curr.marks,0);
console.log(marks);
...
let totalno = studentRecords.filter((item) => item.marks>50).reduce((acc,curr) => acc += curr.marks,0);
